...
sub tell_slack {
	...
	if ( $response->content() ne 'ok' ) {
		print "Returned an error from Slack.\n";
...
sub get_ship {
	...
	if ( ! $response->is_success ) {
		print "Could not pull ship type from EVE api.\n" if $opt{'v'};
		return('Unknown');
	}
...
sub get_config {
	...
	my %validations = (
		...
		slack_hook_url => '^https:\/\/hooks\.slack\.com\/services\/[0-9a-zA-Z]+\/[0-9a-zA-Z]+\/[0-9a-zA-Z]+$',
		channel => '^(#)?[a-zA-Z0-9]+$',
	...
	while ( my ( $key, $value ) = each %validations ) {
		if ( $conf{$key} !~ /$value/ ) {
			die "$key does not have valid input:\n $conf{$key}\n $value\n";
		}
	}
	return(\%conf);
}
